...
public:
  using Move = actions_quiz_msg::action::Distance;
  using GoalHandleMove = rclcpp_action::ClientGoalHandle<Move>;

  explicit MyActionClient(
      const rclcpp::NodeOptions &node_options = rclcpp::NodeOptions())
      : Node("action_quiz_client", node_options), goal_done_(false) {
...
private:
  ...
  void result_callback(const GoalHandleMove::WrappedResult &result) {
    ...
    RCLCPP_INFO(this->get_logger(), "Result received: status(%d) dist(%f)",
